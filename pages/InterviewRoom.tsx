...
const QUESTIONS_COUNT = 5; // STRICT REQUIREMENT

export const InterviewRoom: React.FC<Props> = ({ onComplete }) => {
  const navigate = useNavigate();

  // Setup State
  const [domain, setDomain] = useState<SkillDomain>(SkillDomain.DSA);
  ...
  const submitAnswer = async () => {
    ...
    let result = {
      score: finalTranscript.length > 20 ? 85 : 45,
      feedback: "Answer received and evaluated locally.",
      spokenFeedback: finalTranscript.length > 20 ? "Excellent technical reasoning. Let's move to the next topic." : "Response recorded. I need you to elaborate more on the next one."
    };
